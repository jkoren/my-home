...
const PossessionPage = (props) => {
  ...
  const fetchPossessionAndProfessionalsAndManuals = () => {
    ...
    fetch(`/api/v1/possessions/${id}`, {
      ...
      .then((responseBody) => {
        ...
        setManuals(responseBody.manuals) // turned off to avoid hitting MANUALS webscraper too much
      })
  ...
  const editPossession = (message) => {
    ...
    let updatedPossession = message.possession

    // MESSAGE.POSSESSION IS NEW DATA - going to need to change the Possession UPDATE controller
    
    // updatedPossession.name = message.possession.name
    // updatedPossession.manufacturer = message.possession.manufacturer
    // updatedPossession.model = message.possession.model
    // updatedPossession.owners_manual = message.possession.owners_manual
    // updatedPossession.description = message.possession.description
    // updatedPossession.aws_image = message.possession.aws_image
    // updatedPossession.aws_owners_manual = message.possession.aws_owners_manual
    // updatedPossession.aws_purchase_receipt = message.possession.aws_purchase_receipt
    // updatedPossession.aws_warranty = message.possession.aws_warranty
    // updatedPossession.operating_video = message.possession.operating_video
    // updatedPossession.share_on_new_possession_list = message.possession.share_on_new_possession_list
    // updatedPossession.URL = message.possession.URL
    // updatedPossession.aws_tag = message.possession.aws_tag
